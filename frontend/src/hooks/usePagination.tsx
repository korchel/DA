...
export const usePagination = ({
  ...
  const paginationRange = useMemo(() => {
    ...
    const firstButton = numberOfPages - currentPage > numberOfSiblings
      ? Math.max(currentPage - numberOfSiblings, 1)
      : Math.min(currentPage - numberOfSiblings, numberOfPages - numberOfSiblings * 2);
    const numberOfLeftButtons = Math.min(currentPage - firstButton, numberOfSiblings);
    const numberOfRightButtons = numberOfSiblings * 2 - numberOfLeftButtons;
    const lastButton = Math.min(currentPage + numberOfRightButtons, numberOfPages);

    return [...range(firstButton, currentPage), ...range(currentPage + 1, lastButton)];
